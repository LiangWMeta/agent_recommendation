...
Before running the agent, ensure data is prepared:

```bash
python3 scripts/setup_data.py              # check + build missing stages
python3 scripts/setup_data.py --check-only  # just report status
python3 scripts/setup_data.py --force       # rebuild everything
```

See `data/datasets.md` for the full data pipeline documentation.
...
Generated by `ads_pool/refresh.py` from split data. Gives you semantic understanding of the candidate pool.
- `pool_overview.md` — Pool size, engagement stats, embedding properties
- `catalog.md` — Per-ad summary: engagement count, appearances, embedding norm
- `semantic_clusters.md` — Two-level hierarchical clusters of the ad embedding space

### User Context (`user/{request_id}/`)
Generated per request by `scripts/prepare_contexts.py`. Tells you who the user is.
...
- `context.md` — Request context: task, similarity landscape, pool statistics
